...
class Order(MinimalModel):
    ...
    writer = models.ForeignKey(verbose_name="writer", to=Writer, on_delete=models.SET_DEFAULT,
                               limit_choices_to={'is_active': True, }, default=0,
    ...
    user = models.ForeignKey(verbose_name="user", to=settings.AUTH_USER_MODEL, on_delete=models.SET_DEFAULT,
                             limit_choices_to={'is_active': True, 'is_staff': False}, default=0,
                             help_text=_('Client Placing the order'))
    ...
    preference = models.CharField(choices=PreferencesChoices.choices, default=PreferencesChoices.STANDARD, blank=False,
                                  max_length=20,
    ...
    additional_materials = models.ManyToManyField(Files, blank=True, limit_choices_to={'is_deleted': False})
    ...
    smart = models.BooleanField(_('is_paper'), default=False, help_text=_('Whether an order is  a smart paper.'))
...
class Notification(MinimalModel):
    user = models.ForeignKey(to=settings.AUTH_USER_MODEL, to_field='username', on_delete=models.SET_DEFAULT,
                             default='deleted',
                             limit_choices_to={'is_active': True, 'user_type': 'USER'},
                             help_text=_('User to be sent this notification'))
    content = models.TextField(help_text=_('Notification message'), max_length=1000)
    created = models.ForeignKey(to=settings.AUTH_USER_MODEL, to_field="username", on_delete=models.SET_DEFAULT,
                                related_name='created', related_query_name='creator', default='deleted',
